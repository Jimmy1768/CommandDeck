...
CommandDeck uses this SourceGrid proxy endpoint:
...
SourceGrid later accepted this boundary as its own contract and added guarded
`sourcegrid_dev` dispatch. CommandDeck still keeps the local CLI preview
non-networked; the preview does not call SourceGrid or AppRelay.
